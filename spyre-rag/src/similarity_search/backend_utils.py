...
from similarity_search.reranker_utils import rerank_documents
from similarity_search.retrieval_utils import retrieve_documents
import time
...
def search_only(question, emb_model, emb_endpoint, max_tokens, reranker_model, reranker_endpoint, top_k, top_r, vectorstore, rerank=False, mode="dense"):
    # Perform retrieval
    perf_stat_dict = {}

    start_time = time.time()
    retrieved_documents, retrieved_scores = retrieve_documents(question, emb_model, emb_endpoint, max_tokens,
                                                               vectorstore, top_k, mode)
    perf_stat_dict["retrieve_time"] = time.time() - start_time

    if not rerank:
        return retrieved_documents,retrieved_scores, perf_stat_dict
    ...
    return filtered_docs, filtered_scores, perf_stat_dict
