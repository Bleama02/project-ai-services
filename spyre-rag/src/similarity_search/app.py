...
from similarity_search.backend_utils import search_only, validate_query_length
...
@app.post(
    ...
    description="Uses a dense vector similarity search with an option to use reranking"
)
async def similarity_search(req: SimilaritySearchRequest) -> SimilaritySearchResponse:
    ...
    try:
        ...
        docs, scores, perf_stat_dict = await asyncio.to_thread(
            search_only,
            req.query,
            emb_model,
            emb_endpoint,
    ...
    score_type = "relevance" if req.rerank else "cosine"
    results = [
        SimilaritySearchResult(document=doc, score=score, rank=i)
        for i, (doc, score) in enumerate(zip(docs, scores), 1)
    ]

    return SimilaritySearchResponse(results=results, score_type=score_type, perf_metrics=perf_stat_dict)
